...
namespace XunlanEditor.GameProject
{
    ...
    {
        public static Project CurrProject { get; set; }
        ...
        public string ProjectFilePath { get => Path.Combine(ProjectDirPath, $"{ProjectName}{ProjectFileInfo.ProjectFileSuffix}"); }
        public string SolutionPath { get => Path.Combine(ProjectDirPath, $"{ProjectName}{ProjectFileInfo.SolutionSuffix}"); }
        public string XunlanDirPath { get => Path.Combine(ProjectDirPath, nameof(ProjectFileInfo.ProjectFolders._Xunlan)); }
        public string ContentDirPath { get => Path.Combine(ProjectDirPath, nameof(ProjectFileInfo.ProjectFolders.Content)); }
        public string GameCodeDirPath { get => Path.Combine(ProjectDirPath, nameof(ProjectFileInfo.ProjectFolders.GameCode)); }
        ...
        public string StandAloneBuildConfig { get => BuildConfig == 0 ? nameof(BuildConfiguration.Debug) : nameof(BuildConfiguration.Release); }
        public string DLLBuildConfig { get => BuildConfig == 0 ? nameof(BuildConfiguration.DebugEditor) : nameof(BuildConfiguration.ReleaseEditor); }
        ...
        public string[] AvailableScriptNames
        {
            ...
            set
            {
                if (_availableScriptNames == value) return;
        ...
        public ReadOnlyObservableCollection<Scene> SceneList { get; private set; }

        public Scene ActiveScene { get => SceneList.FirstOrDefault(scene => scene.IsActive); }
        ...
        public ICommand StartWithoutDebugCommand { get; private set; }
        ...
        public void Unload()
        {
            ...
            foreach (Scene scene in SceneList) scene.Unload();
        ...
        {
            string configName = StandAloneBuildConfig;
        ...
        {
            string configName = DLLBuildConfig;
            string dllPath = $@"{ProjectDirPath}\bin\output\x64\{configName}\{ProjectName}.dll";

            if (!File.Exists(dllPath))
            {
                Logger.LogMessage(MsgType.Error, $"{ProjectName}.dll is not found. Please build the project first.");
            }
            else if (EngineAPI.LoadGameCodeDLL(dllPath) != 0)
            {
            ...
            else Logger.LogMessage(MsgType.Error, $"Failed to load {ProjectName}.dll.");
        }
        private void UnloadGameCodeDLL()
        {
            ...
            if (EngineAPI.UnloadGameCodeDLL() != 0) Logger.LogMessage(MsgType.Info, $"{ProjectName}.dll unloaded successfully.");
        }
