...
namespace Xunlan::Tools
{
    namespace
    {
        ...
        Mesh InternalCreatePlane(
            ...
            uint32_t horizontalIndex = Axis::X, uint32_t verticalIndex = Axis::Z, bool flipWinding = false,
            Vector3 offset = { -0.5f, 0.0f, -0.5f }, Vector2 uRange = { 0.0f, 1.0f }, Vector2 vRange = { 0.0f, 1.0f })
        {
            ...
            const uint32_t horizontalCount = Clamp(info.segments[horizontalIndex], 1u, 100u);
            const uint32_t verticalCount = Clamp(info.segments[verticalIndex], 1u, 100u);
            ...
            assert((uint32_t)mesh.positions.size() == (horizontalCount + 1) * (verticalCount + 1));

            // Prepare for rawIndices
            const uint32_t numVerticesInRow = horizontalCount + 1;
            for (uint32_t i = 0; i < verticalCount; ++i)
            {
                for (uint32_t j = 0; j < horizontalCount; ++j)
                {
            ...
            const uint32_t numIndices = { 3 * 2 * horizontalCount * verticalCount };
